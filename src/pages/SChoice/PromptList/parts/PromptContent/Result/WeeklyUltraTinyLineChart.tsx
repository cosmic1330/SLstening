...
const WeeklyUltraTinyLineChart = ({ stock_id }: { stock_id: string }) => {
  ...
  useEffect(() => {
    ...
    const sqlQuery = `SELECT ${IndicatorColor.map((item) => item.key).join(
      ","
    )} FROM weekly_skills JOIN weekly_deal ON weekly_skills.t = weekly_deal.t AND weekly_skills.stock_id = weekly_deal.stock_id WHERE weekly_skills.stock_id = ${stock_id} ORDER BY weekly_skills.t DESC LIMIT 12`;
