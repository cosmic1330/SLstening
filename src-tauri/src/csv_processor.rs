...
pub fn write_entities_to_csv(entities: &[DataEntity], path: &Path) -> Result<(), CsvError> {
    ...
    match &entities[0] {
        DataEntity::DailyDeal(_) => write_deal_csv(entities, path),
        DataEntity::DailySkills(_) => write_skills_csv(entities, path)
    }
}
...
fn write_deal_csv(entities: &[DataEntity], path: &Path) -> Result<(), CsvError> {
    ...
    for entity in entities {
        if let DataEntity::DailyDeal(deal) = entity {
            writer.write_record(&[
                deal.stock_id.clone(),
                deal.t.clone(),
                deal.c.to_string(),
...
fn write_skills_csv(entities: &[DataEntity], path: &Path) -> Result<(), CsvError> {
    ...
    writer.write_record(&[
        ...
        "rsi5", "rsi10", "boll_ub", "boll_ma", "boll_lb", "obv", "obv5"
    ])?;
    
    for entity in entities {
        if let DataEntity::DailySkills(skills) = entity {
            writer.write_record(&[
