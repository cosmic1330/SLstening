...
export default function Bollean({ id }: { id?: string }) {
  ...
  const chartData = useMemo((): BolleanChartData[] => {
    ...
    return baseData.map((d, i) => {
        ...
        if (!isNum(price) || !isNum(ub) || !isNum(lb) || !isNum(ma) || !isNum(width)) return d;
        ...
        const breakoutUp = isSqueeze && price > ub && (d.v || 0) > ((prev.v || 0) * 1.3); // Volume spike
  ...
  const { steps, score, recommendation } = useMemo(() => {
    ...
    // Safety
    if (!isNum(price) || !isNum(ub) || !isNum(lb) || !isNum(ma) || !isNum(width)) {
    ...
    const isWide = width > 0.30; // > 30% as "High/Volatile"
    
    // 2. Trend
    const maSlope = ma - (prev.bollMa || ma);
    const maRising = maSlope > 0;
    const pricePosition = price > ma ? "Bullish (Above MA)" : "Bearish (Below MA)";
    ...
    const bolleanSteps: BolleanStep[] = [
      {
        ...
        checks: [
          { label: `帶寬狀態: ${isSqueeze ? '壓縮 (<15%)' : isWide ? '擴張 (>30%)' : '正常'}`, status: "manual" },
          { label: `趨勢方向: ${maRising ? '上彎 (偏多)' : '下彎 (偏空)'}`, status: maRising ? 'pass' : 'fail' },
          { label: `價格位置: ${pricePosition}`, status: price > ma ? 'pass' : 'fail' },
        ],
      },
      {
        label: "II. 入場條件",
        ...
        checks: [
          { label: "突破上軌且放量 (Squeeze Breakout)", status: (price > ub && volSpike) ? 'pass' : 'fail' },
          { label: "觸及下軌並反轉 (Reversal)", status: reversalCandle ? 'pass' : 'fail' },
          { label: "中軌向上支撐", status: (maRising && price > ma) ? 'pass' : 'fail' },
        ],
      },
      {
        label: "III. 風險控管",
        ...
        checks: [
          ...
          { label: `帶寬: ${(width * 100).toFixed(1)}% (過寬減倉)`, status: isWide ? 'fail' : 'pass' },
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <YAxis domain={["auto", "auto"]} />
             <YAxis yAxisId="right" orientation="right" domain={[0, (dataMax: number) => dataMax * 4]} hide={activeStep < 1} />
