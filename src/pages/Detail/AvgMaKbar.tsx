...
export default function AvgMaKbar({ id }: { id?: string }) {
  ...
  const { steps, score, recommendation } = useMemo(() => {
    ...
    const avgSteps: AvgMaStep[] = [
        ...
        },
        {
            ...
            description: "量價與均線",
            checks: [
                 { label: `價格站穩 EMA5: ${price > ema5 ? "Yes" : "No"}`, status: price > ema5 ? "pass" : "fail" },
                 { label: `成交量 > 均量: ${volOk ? "Yes" : "No"}`, status: volOk ? "pass" : "manual" }
            ]
        },
        {
            label: "IV. 綜合評估",
          ...
          <ComposedChart data={chartData} syncId="avgSync">
            ...
            />
             ...
             {signals.map((signal) => {
               ...
               const yPos = isGolden ? (signal.ema5! - signal.ema5! * 0.01) : (signal.ema5! + signal.ema5! * 0.01);
               
               // Colors
               const color = isGolden ? "#e26d6d" : "#79e26d";  // Red (Up), Green (Down)
                     ...
                     return (
                       <g>
                         {isGolden ? (
                           ...
                           <>
                             <path
                               d={`M${cx - 6},${cy + 10} L${cx + 6},${cy + 10} L${cx},${cy} Z`}
